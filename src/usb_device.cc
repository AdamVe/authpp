...
UsbDevice::UsbDevice(libusb_device* device)
  ...
  for (std::size_t configurationNum = 0;
       ...
       ++configurationNum) {
    if (0 !=
        libusb_get_config_descriptor(device, configurationNum,
                                     &config_descriptors[configurationNum])) {
      ...
      for (int altsettingNum = 0; altsettingNum < usbInterface.num_altsetting;
        ...
        if (iClass == usbClass && iSubClass == usbSubClass) {
          ...
          for (int endpointNum = 0; endpointNum < altsetting.bNumEndpoints;
            ...
            log.v(
                "      Endpoint {} bmAttributtes: {:02x} bEndpointAddress: "
                "{:02x}",
            ...
            if ((endpoint.bmAttributes & 0x03) ==
              ...
              log.v("        searching endpoints (addr={}, dir={}, eaddr={})",
          ...
          if (ein > -1 && eout > -1) {
            log.v(
                "Found {:02x}:{:02x} with on interface {} with altsetting {} "
